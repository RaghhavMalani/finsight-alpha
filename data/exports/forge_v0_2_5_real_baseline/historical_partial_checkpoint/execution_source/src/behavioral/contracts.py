...
class ModelIdentity:
    ...
    cached_input_usd_per_million_tokens: float | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.cached_input_usd_per_million_tokens is not None:
            _number(self.cached_input_usd_per_million_tokens, "cached input price")
            if self.cached_input_usd_per_million_tokens > self.input_usd_per_million_tokens:
                raise ValueError("cached price exceeds input price")
        object.__setattr__(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            **({"reasoning_effort": self.reasoning_effort,
                "cached_input_usd_per_million_tokens": self.cached_input_usd_per_million_tokens}
               if self.reasoning_effort is not None or self.cached_input_usd_per_million_tokens is not None else {}),
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "ModelIdentity":
        ...
        optional = {"reasoning_effort", "cached_input_usd_per_million_tokens"}
        return cls(**_exact(value, fields | (set(value) & optional), "model identity"))
