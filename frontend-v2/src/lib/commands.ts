...
export const COMMANDS: CommandDef[] = [
  { code: "HOME", label: "Home", description: "Market overview dashboard", keywords: ["dashboard", "overview", "start", "brief"] },
  { code: "MK", label: "Markets", description: "Live price, indicators, depth", keywords: ["market", "price", "quote", "depth", "chart"] },
  { code: "OC", label: "Options", description: "Chain, walls, strategy builder", keywords: ["option", "chain", "greek", "call", "put", "strategy", "spread"] },
  { code: "MC", label: "Monte Carlo", description: "3D probability landscape", keywords: ["monte", "simulation", "paths", "sim", "gbm", "landscape"] },
  { code: "GR", label: "Greeks", description: "3D greek surfaces", keywords: ["greek", "delta", "gamma", "vega", "theta", "surface"] },
  ...
  { code: "ALT", label: "Alternative data", description: "Shipping · weather · EV · card · satellite · web", keywords: ["alt", "alternative", "shipping", "weather", "tesla", "deliveries", "card", "spend", "satellite", "parking", "web", "traffic", "kaggle"] },
  ...
  { code: "SIGHT", label: "AI research", description: "Ask the desk", keywords: ["ai", "ask", "research", "assistant", "brief"] },
  { code: "TOUR", label: "Tour", description: "Replay the guided tour", keywords: ["tour", "help", "onboard", "guide", "spotlight"] },
];
...
const NL_HINTS: Array<{ match: RegExp; code: string }> = [
  ...
  { match: /\b(depend|supplier|customer|competitor|shock|propagat|elasticity|impact)\b/i, code: "CX" },
  ...
  { match: /\b(alt|alternative|shipping|port|weather|hdd|cdd|drought|tesla\s*deliver|card\s*spend|satellite|parking|web\s*traffic|kaggle|dataset)\b/i, code: "ALT" },
...
export function parseCommand(raw: string): ParsedCommand | null {
  ...
  const vsMatch = upperAll.match(/\b([A-Z][A-Z0-9-]*)\s+(?:VS|V|VERSUS)\s+([A-Z][A-Z0-9-]*)\b/);
  if (vsMatch && TICKERS.includes(vsMatch[1]) && TICKERS.includes(vsMatch[2])) {
  ...
  if (cmpMatch && TICKERS.includes(cmpMatch[1])) {
    return { code: "HOME", symbol: cmpMatch[1], symbol2: cmpMatch[2] && TICKERS.includes(cmpMatch[2]) ? cmpMatch[2] : undefined, action: "COMPARE" };
  }

  const tokens = upperAll.split(/\s+/);
  const foundSymbol = tokens.find((t) => TICKERS.includes(t));
