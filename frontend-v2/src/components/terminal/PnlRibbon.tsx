...
export function PnlRibbon({
  ...
  const { sessionPnl, sessionPct, notional, unrealized } = useMemo(() => {
    let notional = 0, sessionPnl = 0, unrealized = 0, prevNotional = 0;
    ...
    return (
      ...
      >
        <div className="mono-caps flex items-center justify-between text-[9px] text-faint">
          ...
          <span className="text-primary">EMPTY · CLICK TO ADD</span>
        </div>
        <div className="mt-1 font-mono text-xl text-muted-foreground">+$0</div>
  ...
  return (
    ...
    >
      ...
      <svg viewBox="0 0 240 18" preserveAspectRatio="none" className="mt-1 h-3 w-full">
        <line x1="0" x2="240" y1={sessionPnl >= 0 ? 16 : 2} y2={sessionPnl >= 0 ? 16 : 2} stroke="#232830" strokeDasharray="2 3" strokeWidth="1" />
        {sparkPts && (
          <polyline points={sparkPts} fill="none" stroke={up ? "#42C98B" : "#F06464"} strokeOpacity="0.55" strokeWidth="1" vectorEffect="non-scaling-stroke" />
        )}
      </svg>

      <div className="mono-caps mt-1.5 flex items-center gap-2 text-[9px] text-faint tabular-nums">
        <span className={rangeLo < 0 ? "text-down" : "text-foreground"}>{rangeLo >= 0 ? "+" : "−"}${fmt(Math.abs(rangeLo), 0)}</span>
        <div className="relative h-[3px] flex-1 bg-background">
          ...
          <div
            ...
            style={{ left: `calc(${rangePos * 100}% - 4px)`, transition: "left 400ms cubic-bezier(0.16,1,0.3,1)" }}
          />
        </div>
        <span className={rangeHi >= 0 ? "text-up" : "text-foreground"}>{rangeHi >= 0 ? "+" : "−"}${fmt(Math.abs(rangeHi), 0)}</span>
      </div>
      ...
      <span className="sr-only">Unrealized {unrealized >= 0 ? "+" : "-"}${fmt(Math.abs(unrealized), 0)}</span>
    </button>
  );
}
