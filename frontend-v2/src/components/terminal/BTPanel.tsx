...
import { serverBacktestView } from "@/lib/server-backtest";
...
export function BTPanel({
  ...
  async function run() {
    ...
    try {
      ...
      const remote = await runApiBacktest(strat, ticker, years);
      setRemoteResult(remote);
      setResult(serverBacktestView(remote, strat.name));
      setPhase("compute");
    ...
    <div className="flex h-full flex-col overflow-hidden">
        ...
        <div className="mono-caps border-b border-primary/40 bg-primary/5 px-3 py-2 text-[10px]">
          {lines.map((l, i) => (
            <div
              ...
              style={{ animation: "fade-in 220ms ease-out both", opacity: 0.9 }}
            >
      ...
      {remoteResult && !running && (
        <div className="mono-caps flex items-center gap-4 border-b border-up/40 bg-up/5 px-3 py-2 text-[9px] text-up">
          <span>{remoteResult.truth.state} · SERVER</span>
          <span className="text-foreground">AS OF {remoteResult.truth.as_of.slice(0, 10)}</span>
          <span className="text-faint">DATA THROUGH {remoteResult.truth.observed_through}</span>
          <span className="text-foreground">{remoteResult.n_trades} TRADES</span>
          <span className="text-foreground">
            SHARPE {Number(remoteResult.stats.sharpe ?? 0).toFixed(2)}
          </span>
          <span className="text-foreground">
            OOS {Number(remoteResult.out_of_sample.sharpe ?? 0).toFixed(2)}
          </span>
          <span className="ml-auto text-faint" title={remoteResult.truth.run_id}>
            RUN {remoteResult.truth.run_id.slice(0, 10)}
          </span>
        </div>
      )}
      <div className="flex-1 overflow-y-auto p-3">
        {result ? (
          <>
            <div className="grid grid-cols-12 gap-3">
              <div className="col-span-12">
                <EquityChart key={runId} r={result} showOOS={showOOS} />
              </div>
              <div className="col-span-12">
                <UnderwaterChart r={result} showOOS={showOOS} />
              </div>
              <div className="col-span-12">
                <PriceWithTrades r={result} />
              </div>
              <div className="col-span-7">
                <StatsGrid r={result} showOOS={showOOS} runId={runId} />
              </div>
              <div className="col-span-5">
                <MonthlyHeatmap r={result} />
              </div>
            </div>
            <div
              className="mono-caps mt-4 border-l-2 border-primary bg-primary/5 px-3 py-2 text-[11px] text-foreground"
              style={{ animation: `fade-in 400ms ease-out both`, animationDelay: "300ms" }}
              key={runId}
            >
          ...
          <div className="mono-caps flex h-full min-h-64 items-center justify-center border border-divider bg-panel text-center text-[11px] text-faint">
            <div>
              <span className="text-primary">UNAVAILABLE · </span>Run the server calculation to
...
function verdict(r: BacktestResult) {
  ...
  parts.push(
    `IS Sharpe ${is.toFixed(2)} → OOS Sharpe ${oos.toFixed(2)}${decay ? " — edge decays out-of-sample, likely overfit." : " — edge holds out-of-sample."}`,
  );
  if (Math.abs(r.stats.maxDD) > 0.3)
    parts.push(
      `Deep drawdowns (${(r.stats.maxDD * 100).toFixed(0)}%) — sizing / stops need review.`,
    );
  return parts.join(" ");
}
...
function Toolbar({
  ...
  ticker: string;
  onTicker: (s: string) => void;
  years: 1 | 3 | 5;
  onYears: (y: 1 | 3 | 5) => void;
  showOOS: boolean;
  onOOS: (b: boolean) => void;
  running: boolean;
  onRun: () => void;
}) {
  return (
    <div className="mono-caps flex flex-wrap items-center gap-2 border-b border-divider bg-panel px-3 py-2 text-[10px]">
      ...
      <select
        ...
        className="interactive border border-border bg-background px-2 py-1 text-[10px] text-foreground"
      >
        {strategies.map((s) => (
      ...
      <select
        ...
        className="interactive border border-border bg-background px-2 py-1 text-[10px] text-foreground"
      >
        {TICKERS.filter((t) => t !== "BTC-USD").map((t) => (
      ...
      <div className="flex gap-0.5">
        {([1, 3, 5] as const).map((y) => (
          <button
            key={y}
            onClick={() => onYears(y)}
            className={`interactive border px-2 py-1 ${years === y ? "border-primary text-primary bg-primary/10" : "border-border text-faint hover:text-foreground"}`}
          >
            {y}Y
          </button>
        ))}
      ...
      <button
        onClick={() => onOOS(!showOOS)}
        className={`interactive ml-2 border px-2 py-1 ${showOOS ? "border-info text-info" : "border-border text-faint hover:text-foreground"}`}
      >
        WALK-FWD 70/30
      </button>
      <button
        onClick={onRun}
        disabled={running}
        className="mono-caps interactive ml-auto border border-primary bg-primary/10 px-4 py-1 text-primary hover:bg-primary hover:text-primary-foreground disabled:opacity-40"
      >
        {running ? "RUNNING…" : "RUN ▶"}
      </button>
    </div>
    ...
    <div className="border border-divider bg-panel">
      <div className="mono-caps flex items-center justify-between border-b border-divider px-3 py-1.5 text-[10px]">
        <span className="text-primary">
          EQUITY CURVE · {r.ticker} · {r.strategy}
        </span>
        <span className="flex gap-3 text-[9px]">
          ...
          {showOOS && (
            <span className="flex items-center gap-1">
              <span className="inline-block h-2 w-4 bg-info/20" />
              OOS
            </span>
          )}
        </span>
      ...
      <svg viewBox="0 0 1000 140" className="h-40 w-full" preserveAspectRatio="none">
        {showOOS && (
          <rect x={oosX} y={0} width={1000 - oosX} height={140} fill="#45B9D3" opacity={0.08} />
        )}
        <polyline
          points={benchPts}
          fill="none"
        ...
        <polyline
          ...
          style={{
            ...
            strokeDashoffset: 0,
            animation: "draw-bt 800ms cubic-bezier(0.16,1,0.3,1) both",
          }}
        />
        <line
          x1={0}
          y1={140 - ((1 - min) / (max - min || 1)) * 130 - 5}
          x2={1000}
          y2={140 - ((1 - min) / (max - min || 1)) * 130 - 5}
          stroke="#171B1F"
          strokeDasharray="2 3"
        />
  ...
  return (
    <div className="border border-divider bg-panel">
      ...
      <svg viewBox="0 0 1000 60" className="h-14 w-full" preserveAspectRatio="none">
        {showOOS && (
          <rect x={oosX} y={0} width={1000 - oosX} height={60} fill="#45B9D3" opacity={0.08} />
        )}
  ...
  return (
    <div className="border border-divider bg-panel">
      ...
      <svg viewBox="0 0 1000 100" className="h-24 w-full" preserveAspectRatio="none">
        ...
        />
        {r.trades.map((t, i) => {
          const eI = tByT.get(t.entryT);
          const xI = tByT.get(t.exitT);
          if (eI === undefined || xI === undefined) return null;
          const e = s(t.entryPx, eI);
          const x = s(t.exitPx, xI);
          return (
            <g key={i}>
              <text x={e.x} y={e.y + 8} fontSize={7} fill="#42C98B" textAnchor="middle">
                ▲
              </text>
              <text
                x={x.x}
...
function StatsGrid({ r, showOOS, runId }: { r: BacktestResult; showOOS: boolean; runId: number }) {
  const cells = [
    ...
    { l: "SORTINO", v: r.stats.sortino, fmt: (v: number) => v.toFixed(2), oos: r.oosStats.sortino },
    {
      ...
      v: r.stats.maxDD * 100,
      fmt: (v: number) => `${v.toFixed(1)}%`,
      oos: r.oosStats.maxDD * 100,
    },
    {
      l: "WIN RATE",
      v: r.stats.winRate * 100,
      fmt: (v: number) => `${v.toFixed(0)}%`,
      oos: r.oosStats.winRate * 100,
    },
    {
      l: "PROFIT FACTOR",
      v: Math.min(9.99, r.stats.profitFactor),
      fmt: (v: number) => v.toFixed(2),
      oos: Math.min(9.99, r.oosStats.profitFactor),
    },
    { l: "TRADES", v: r.stats.trades, fmt: (v: number) => v.toFixed(0), oos: r.oosStats.trades },
    {
      l: "EXPOSURE",
      v: r.stats.exposure * 100,
      fmt: (v: number) => `${v.toFixed(0)}%`,
      oos: r.oosStats.exposure * 100,
    },
  ];
  return (
    <div className="border border-divider bg-panel">
...
function StatCell({
  ...
  const color =
    value < 0 || (label === "MAX DD" && value < 0)
      ? "text-down"
      : label === "SHARPE" || label === "SORTINO"
        ? value > 1
          ? "text-up"
          : "text-foreground"
        : "text-foreground";
...
function MonthlyHeatmap({ r }: { r: BacktestResult }) {
  ...
  r.monthlyReturns.forEach((m) => {
    grid[`${m.year}-${m.month}`] = m.ret;
  });
  const all = r.monthlyReturns.map((m) => m.ret);
  const absMax = Math.max(...all.map((v) => Math.abs(v)), 0.001);
  return (
    <div className="border border-divider bg-panel">
      <div className="mono-caps border-b border-divider px-3 py-1.5 text-[10px] text-primary">
        MONTHLY RETURNS
      </div>
      <div className="p-3">
        <div
          className="mono-caps grid text-[8px] text-faint"
        ...
        {years.map((y) => (
          <div key={y} className="grid" style={{ gridTemplateColumns: `28px repeat(12, 1fr)` }}>
            <span className="mono-caps text-[8px] text-faint self-center">
              {String(y).slice(-2)}
            </span>
            {MONTHS.map((_, mi) => {
              const v = grid[`${y}-${mi}`];
              ...
              return (
                <div
                  key={mi}
                  title={v !== undefined ? `${y}-${mi + 1} · ${(v * 100).toFixed(2)}%` : "—"}
                  className="m-[1px] h-4"
                  style={{ background: bg }}
                />
              );
