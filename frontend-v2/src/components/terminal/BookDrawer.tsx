...
export function BookDrawer({
  ...
  const suggestions = useMemo(() => {
    ...
    if (!upper) return [];
    const directory = [...TICKERS, ...Object.keys(COMMODITIES)];
    return directory.filter((t) => t.startsWith(upper) && !owned.has(t)).slice(0, 8);
  }, [q, positions]);
    ...
    return (
      <div className="border border-primary/40 bg-panel p-4 amber-glow">
        <div className="mono-caps mb-2 flex items-center gap-2 text-[10px]">
          ...
          <span
            className={`border px-1.5 py-0.5 text-[8px] ${sync.state === "offline" ? "border-down/50 text-down" : "border-border text-faint"}`}
          >
            {sync.state === "synced"
              ? "SERVER SYNCED"
              : sync.state === "saving"
        ...
        <div className="font-serif text-[13px] text-foreground leading-snug">
          No fabricated positions are loaded. Add a paper position to start tracking P&amp;L and
          risk.
        </div>
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <div className="w-full border border-divider bg-background/50 p-2">
            <div className="mono-caps mb-2 text-[8px] text-faint">QUICK COMMODITY CONTRACTS</div>
            <div className="flex flex-wrap gap-1">
              {Object.values(COMMODITIES).map((commodity) => (
                <button
                  key={commodity.symbol}
                  onClick={() => commitAdd(commodity.symbol, 1)}
                  className="mono-caps interactive border border-border px-2 py-1 text-[8px] text-muted-foreground hover:border-up/50 hover:text-up"
                >
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          {edit && (
            <>
              ...
              <button
                onClick={() => {
                  if (confirm("Clear all positions?")) clearDemoBook();
                }}
      ...
      <div className="max-h-[260px] overflow-y-auto divide-y divide-divider/60">
        {rows.map(({ p, last, dayPnl, unrl, mv }) => {
          ...
          return (
            ...
            >
              <span
                className={`mono-caps rounded-sm px-1 py-0.5 text-[8px] text-center ${clsColor}`}
              >
                {c}
              </span>
              <span className={`mono-caps text-[11px] ${short ? "text-down" : "text-foreground"}`}>
                {p.symbol}
              </span>
              {edit ? (
                <input
