...
function ProbabilityChart({ payload }: { payload: SignalPayload }) {
  const points = payload.prob_timeline.prob_up
    ...
    .filter(
      (point): point is { value: number; date: string } =>
        point.value != null && Number.isFinite(point.value),
    );

  if (points.length < 2) {
    return (
      <div className="mono-caps p-4 text-[10px] text-faint">
        NO OUT-OF-SAMPLE PROBABILITY TIMELINE RETURNED
      </div>
    );
  }
  ...
  return (
    <div className="border border-divider bg-panel">
      ...
      <svg
        ...
        aria-label="Out-of-sample probability of an upward move"
      >
        <line
          x1={pad}
          x2={width - pad}
...
export function MLSignalsLive({ symbol }: { symbol: string }) {
  const query = useQuery({
    ...
    queryFn: () =>
      api<SignalPayload>(
        `/ml/signal/${encodeURIComponent(symbol)}?benchmark=SPY&horizon=1&as_of=${new Date().toISOString().slice(0, 10)}`,
      ),
    staleTime: 30 * 60_000,
    retry: 0,
  });
    ...
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="border border-divider bg-panel p-5 text-center">
          <div className="mono-caps text-[11px] text-primary">
            TRAINING AND VALIDATING · {symbol}
          </div>
          <div className="mt-2 max-w-md text-[11px] leading-relaxed text-muted-foreground">
            Fetching market history, engineering features, and running the backend model suite. The
            first request can take longer; results are cached for 30 minutes.
          </div>
        </div>
      </div>
    );
  ...
  if (query.isError || !query.data) {
    const message =
      query.error instanceof Error
        ? query.error.message
        : "The ML endpoint did not return a result.";
    return (
      <div className="flex h-full items-center justify-center p-6">
  ...
  const maxImportance = Math.max(
    0.000001,
    ...data.top_features.map((item) => Math.abs(item.importance ?? 0)),
  );
  ...
  return (
    <div className="h-full overflow-y-auto p-3">
      ...
      <div className="grid gap-3 xl:grid-cols-[1.15fr_1fr]">
        <div className="border border-divider bg-panel p-4">
          ...
          <div className="mono-caps mt-4 flex flex-wrap gap-2 text-[9px]">
            <span className="border border-divider px-2 py-1 text-foreground">
              STRENGTH {data.signal.strength ?? "—"}
            </span>
            <span className="border border-divider px-2 py-1 text-foreground">
              CONFIDENCE {data.signal.confidence_band ?? "—"}
            </span>
            <span className="border border-divider px-2 py-1 text-foreground">
              VALIDATION {data.signal.validation_quality ?? "—"}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Metric
            label="ROC AUC"
            value={pct(data.validation.roc_auc)}
            tone={(data.validation.roc_auc ?? 0) >= 0.55 ? "text-up" : "text-foreground"}
          />
          <Metric
            label="MODEL EDGE"
            value={pct(data.validation.model_edge)}
            tone={(data.validation.model_edge ?? 0) > 0 ? "text-up" : "text-down"}
          />
          ...
          <Metric
            ...
            value={data.signal.allowed ? "YES" : "NO"}
            tone={data.signal.allowed ? "text-up" : "text-muted-foreground"}
          />
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-3 xl:grid-cols-[1.15fr_1fr]">
        ...
        <div className="border border-divider bg-panel">
          <div className="mono-caps border-b border-divider px-3 py-1.5 text-[9px] text-primary">
            MARKET CONTEXT · LATEST OBSERVATION
          </div>
          <div className="grid grid-cols-2 gap-2 p-3">
            ...
            <Metric
              label="52W DRAWDOWN"
              value={pct(data.market_context.drawdown_from_52w_high)}
              tone="text-down"
            />
        ...
        <div className="border border-divider bg-panel">
          ...
          <div className="space-y-2 p-3">
            {data.top_features.length === 0 && (
              <div className="text-[10px] text-faint">No feature importance returned.</div>
            )}
            {data.top_features.map((item, index) => {
              ...
              return (
                <div
                  ...
                  className="grid grid-cols-[140px_1fr_60px] items-center gap-2"
                >
                  <span
                    className="truncate font-mono text-[9px] text-muted-foreground"
                    title={item.feature}
                  >
                    {item.feature ?? "unnamed"}
                  </span>
                  <div className="h-2 bg-background">
                    <div
                      className={`h-full ${importance >= 0 ? "bg-primary" : "bg-down"}`}
                      style={{
                        width: `${Math.max(1, (Math.abs(importance) / maxImportance) * 100)}%`,
                      }}
                    />
                  </div>
                  <span className="text-right font-mono text-[9px] tabular-nums text-foreground">
        ...
        <div className="overflow-x-auto border border-divider bg-panel">
          ...
          <table className="w-full min-w-[620px] text-left">
            <thead className="mono-caps text-[8px] text-faint">
              <tr>
                {["MODEL", "ACC.", "PREC.", "RECALL", "F1", "ROC AUC", "BRIER", "EDGE"].map(
                  (label) => (
      ...
      <div className="mono-caps mt-3 border-l-2 border-primary bg-primary/5 px-3 py-2 text-[9px] text-muted-foreground">
        RESEARCH OUTPUT · VALIDATION METRICS ARE REPORTED BY THE BACKEND MODEL SUITE · NOT AN
