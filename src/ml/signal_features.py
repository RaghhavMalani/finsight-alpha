...
def create_signal_research_features(
    ...
    df["close_position_in_range"] = np.where(
        range_diff == 0, 0, (df["Close"] - df["Low"]) / range_diff
    )
    ...
    for window in [5, 10, 20, 30, 60]:
        df[f"realized_vol_{window}"] = df["log_return"].rolling(window).std() * np.sqrt(
            252
        )
    ...
    df["volume_zscore_20"] = np.where(
        vol_std_20 == 0, 0, (df["Volume"] - df["volume_ma_20"]) / vol_std_20
    )
    ...
    df["bollinger_percent_b"] = np.where(
        bb_range == 0, 0, (df["Close"] - df["bollinger_lower_20"]) / bb_range
    )
    ...
    if benchmark_df is not None and not benchmark_df.empty:
        ...
        if "Date" in df.columns:
            ...
            df["asset_minus_benchmark_return"] = (
                df["simple_return"] - df["benchmark_return"]
            )
            ...
            for window in [20, 60]:
                bench_mom = (
                    df["benchmark_close"] / df["benchmark_close"].shift(window) - 1
                )
    ...
    # 11. Regime features
    # volatility_regime
    # Expanding thresholds use only observations available before each row.
    # Global qcut would let the future volatility distribution relabel history.
    historical_vol = df["realized_vol_20"].shift(1)
    low_threshold = historical_vol.expanding(min_periods=60).quantile(1 / 3)
    high_threshold = historical_vol.expanding(min_periods=60).quantile(2 / 3)
    current_vol = df["realized_vol_20"]
    regimes = np.select(
        [current_vol <= low_threshold, current_vol >= high_threshold],
        ["Low", "High"],
        default="Medium",
    )
    df["volatility_regime"] = pd.Series(regimes, index=df.index, dtype="object")
    df.loc[
        current_vol.isna() | low_threshold.isna() | high_threshold.isna(),
        "volatility_regime",
    ] = "Unknown"
    ...
    df["volume_regime"] = np.where(
        df["volume_zscore_20"] > 1.5, "High Volume", "Normal"
    )
