...
def time_series_train_test_split(
    ...
    *,
    ...
    if target_horizon < 0 or embargo < 0:
        raise ValueError("target_horizon and embargo cannot be negative.")
    n = len(df)
    split_idx = int(n * (1 - test_size))
    train_end = split_idx - target_horizon
    test_start = split_idx + embargo
    if train_end <= 0 or test_start >= n:
        raise ValueError("Purge/embargo leaves an empty train or test split.")
...
def walk_forward_validation(
    ...
    if start_idx >= n:
        raise ValueError(
            "initial_train_size is too large, leaving no data for validation."
        )
    ...
    for fold, current_idx in enumerate(range(start_idx, n, step_size)):
        train_end = current_idx - target_horizon
        test_start = current_idx + embargo
        if train_end <= 0 or test_start >= n:
            continue
        train_df = df.iloc[:train_end]

        # Determine the end of the test window for this fold
        end_idx = min(test_start + step_size, n)
