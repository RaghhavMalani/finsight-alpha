...
def train_signal_model_suite(
    ...
    results_df = results_df.sort_values(
        by=["roc_auc", "brier_score", "f1_score"], ascending=[False, True, False]
    )
    ...
    actual_model = (
        diagnostic_model.named_steps["clf"]
        if diagnostic_model_name == "logistic_regression"
        else diagnostic_model
    )
    ...
    model_stress = stress_model_suite(trained_models, X_test, y_test)
