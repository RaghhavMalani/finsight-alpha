...
class OpenAIFactory:
    ...
    def before_tier(self, identity, records, remaining_episodes):
        ...
        interrupted, overhead = recovery_info(self.ledger.path.parent, records)
        if set(settled) != bound_calls | set(interrupted):
        ...
        if settled:
            report = verify_ledger_artifact(self.ledger.path.parent, {
                ...
                "total_cost_usd": overhead + sum(r["run"]["usage"]["inference_cost_usd"] for r in records),
            }, records)
            if report:
...
def verify_ledger_artifact(artifact, manifest, records):
    ...
    from src.behavioral.recovery import recovery_info
    interrupted, _ = recovery_info(artifact, records)
    for key, event in interrupted.items():
        if key in bound or settled[key] != event:
            errors.append("interrupted_request_binding")
        evidence = event["evidence"]
        raw = json.loads(evidence["raw_response"])
        identity = next(m for m in frozen_models() if m.model == event["model"])
        inputs, _, outputs, _ = parse_usage(raw)
        turn = dict(api_evidence=evidence, provider_request_id=evidence["provider_request_id"],
                    tokens_in=inputs, tokens_out=outputs, cost_usd=event["cost_usd"],
                    calculated_cost_usd=event["cost_usd"], text=raw["choices"][0]["message"].get("content") or "")
        if not verify_api_evidence(identity, turn):
            errors.append("interrupted_request_metering")
    if len(bound) != len(set(bound)) or set(bound) | set(interrupted) != set(settled):
