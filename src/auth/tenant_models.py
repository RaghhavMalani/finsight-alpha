...
class DatasetVersion(Base):
    ...
    __table_args__ = (
        UniqueConstraint("dataset_id", "version_hash", name="uq_dataset_version_hash"),
    )
    ...
    available_from: Mapped[dt.datetime] = mapped_column(
...
class DatasetLicense(Base):
    ...
    __table_args__ = (
        UniqueConstraint("dataset_id", "license_id", name="uq_dataset_license"),
    )
    ...
    license_id: Mapped[int] = mapped_column(
        ForeignKey("data_licenses.id", ondelete="CASCADE"), nullable=False
    )
...
class OrganizationLicenseGrant(Base):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "dataset_id", name="uq_org_dataset_grant"),
    )
...
class AnalysisRun(Base):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "run_id", name="uq_analysis_run_org_id"),
    )
    ...
    analysis_type: Mapped[str] = mapped_column(String(96), index=True, nullable=False)
    epistemic_state: Mapped[str] = mapped_column(String(32), nullable=False)
...
class ForecastSignal(Base):
    ...
    signal_date: Mapped[dt.datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    executable_from: Mapped[dt.datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    ...
    signal_label: Mapped[str | None] = mapped_column(String(64))
    status: Mapped[str] = mapped_column(
        String(32), index=True, nullable=False, default="issued"
    )
